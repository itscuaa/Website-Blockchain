...
function showPage(pageId) {
  ...
  document.getElementById("tab-" + pageId.split("-")[1]).classList.add("active");
}

// ▶ Tambahan satu kata "about" TANPA mengubah fungsi asli
...
function updateBalancesDOM() {
  ["A", "B", "C"].forEach((u) => {
    document.getElementById("saldo-" + u).textContent = balances[u];
  });
}
  ...
  ["A", "B", "C"].forEach((u) => {
    ...
    chainsConsensus[u].forEach((blk, i) => {
      ...
      d.innerHTML = `
        ...
        <textarea class="data" rows="3">${blk.data}</textarea>
...
document.getElementById("btn-verify-consensus").onclick = async () => {
  try {
    ...
    alert("Terjadi kesalahan saat verifikasi. Cek console.");
  }
};
